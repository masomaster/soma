...
# ``Asleep`` here is HealthKit's *uncategorized* phase (not a night total).
_SLEEP_ASLEEP_VALUES: frozenset[str] = frozenset(
    {"asleep", "core", "deep", "rem", "unspecified"}
)
_SLEEP_DEEP_VALUES: frozenset[str] = frozenset({"deep"})
_SLEEP_REM_VALUES: frozenset[str] = frozenset({"rem"})
...
def _parse_sample_datetime(raw: str) -> datetime | None:
    """Parse HAE timestamps to aware/naive datetime (date-only → midnight)."""
    if not isinstance(raw, str):
        return None
    text = raw.strip()
    if not text:
...
def _normalize_sleep_value_label(raw: object) -> str:
    if not isinstance(raw, str):
        return ""
    return " ".join(raw.strip().lower().replace("_", " ").replace("-", " ").split())


def _sleep_entry_is_aggregated(entry: Mapping[str, Any]) -> bool:
    """True when the row carries HAE aggregated totals (``totalSleep`` / ``asleep``)."""
...
def _unaggregated_sleep_interval(
    entry: Mapping[str, Any],
    *,
    units: str | None,
) -> tuple[datetime, datetime, float, str] | None:
    """Parse one unaggregated HAE sleep sample → (start, end, hours, stage label)."""
    label = _normalize_sleep_value_label(entry.get("value"))
    if not label:
        return None
    hrs = _sleep_stage_hours(_qty_from_entry(entry), units=units)
    if hrs is None:
        return None
    ...
    if isinstance(end_raw, str):
        end_dt = _parse_sample_datetime(end_raw)
    if end_dt is None:
        # Date-only / missing end: treat qty as a point sample ending at start.
        end_dt = start_dt
    if end_dt < start_dt:
...
def _emit_sleep_session_samples(
    sleep_hours: float,
    deep_hrs: float,
    rem_hrs: float,
    session_date: date,
) -> list[tuple[date, str, float, str | None]]:
    """Emit canonical sleep metrics for one rolled-up night."""
    out: list[tuple[date, str, float, str | None]] = []
    if sleep_hours > 0:
        out.append((session_date, "sleep_hours", round(sleep_hours, 4), "h"))
    if deep_hrs > 0 and "sleep_deep_hrs" in DAILY_HEALTH_METRIC_COLUMNS:
        out.append((session_date, "sleep_deep_hrs", round(deep_hrs, 4), "h"))
    if rem_hrs > 0 and "sleep_rem_hrs" in DAILY_HEALTH_METRIC_COLUMNS:
        out.append((session_date, "sleep_rem_hrs", round(rem_hrs, 4), "h"))
    return out


def _rollup_unaggregated_sleep_sessions(
    intervals: list[tuple[datetime, datetime, float, str]],
) -> list[tuple[date, str, float, str | None]]:
    """Cluster unaggregated stage intervals into nights; attribute to wake morning.

    Health Sync / Fitbit / Google Fit often land in HealthKit as many short
    category samples. HAE's *unaggregated* export mirrors that (``qty`` +
    ``value``). Soma matches aggregated HAE semantics by summing asleep stages
    per contiguous session and dating the night by the last asleep interval's
    end calendar date (wake morning).
    """
    if not intervals:
        return []

    ordered = sorted(intervals, key=lambda item: (item[0], item[1]))
    sessions: list[list[tuple[datetime, datetime, float, str]]] = []
    current: list[tuple[datetime, datetime, float, str]] = []
    prev_end: datetime | None = None

    for start_dt, end_dt, hrs, label in ordered:
        if current and prev_end is not None:
            ...
            if gap_hours > _SLEEP_SESSION_GAP_HOURS:
                ...
                current = []
        current.append((start_dt, end_dt, hrs, label))
    ...
    for session in sessions:
        ...
        for _start, end_dt, hrs, label in session:
            if label in _SLEEP_ASLEEP_VALUES:
                sleep_hours += hrs
                if wake_end is None or end_dt > wake_end:
                    wake_end = end_dt
            if label in _SLEEP_DEEP_VALUES:
                deep_hrs += hrs
            if label in _SLEEP_REM_VALUES:
                rem_hrs += hrs
        if sleep_hours <= 0 or wake_end is None:
            continue
        out.extend(
            _emit_sleep_session_samples(sleep_hours, deep_hrs, rem_hrs, wake_end.date())
        )
    return out
...
def _iter_sleep_analysis_samples(
    ...
    *,
    ...
    for entry in data:
        ...
        if _sleep_entry_is_aggregated(entry):
        ...
        # Aggregated-shaped row that only carries nested deep/rem (no total).
        has_stage_keys = any(_num(entry.get(k)) is not None for k in _SLEEP_STAGE_ROW_KEYS)
        label = _normalize_sleep_value_label(entry.get("value"))
        if has_stage_keys and not label:
            d_raw = entry.get("date") or entry.get("startDate") or entry.get("sleepStart")
            if not isinstance(d_raw, str):
                skipped += 1
                continue
        ...
        interval = _unaggregated_sleep_interval(entry, units=units)
        if interval is not None:
            unagg.append(interval)
            continue

        # Last-resort: qty-only sample with a parseable date (no categorical value).
        qty_hrs = _sleep_stage_hours(_qty_from_entry(entry), units=units)
        if qty_hrs is None:
            skipped += 1
            continue
        d_raw = entry.get("date") or entry.get("startDate") or entry.get("sleepStart")
        ...
        if d is None:
            skipped += 1
            continue
        out.append((d, "sleep_hours", qty_hrs, "h"))

    out.extend(_rollup_unaggregated_sleep_sessions(unagg))
    if data and not out:
        logger.warning(
            "HAE sleep_analysis produced no biometrics rows "
            "(entries=%s skipped=%s unaggregated_intervals=%s) — "
            "check aggregation toggle or sample shape (totalSleep vs qty+value)",
            len(data),
            skipped,
            len(unagg),
        )
