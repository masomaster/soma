...
Use this when choosing **skills**, **subagents**, or **MCP** tools so work stays consistent with [`.cursor/rules/soma.mdc`](.cursor/rules/soma.mdc). For Python, follow **§ Style** there (including **f-strings** — no `%d` / `%`-style string formatting).
...
- `make cdk-synth` is optional; it installs the `.[cdk]` extra and shells out to `npx aws-cdk@2` (Node is already present). It only synthesizes templates — it does not touch any AWS account. The briefing Lambda **layer** is built with **local** `pip` (Python 3.14 + PyPI; no Docker).
