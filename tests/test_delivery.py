...
def test_cloud_env_sends_email_without_prefix():
...
def test_cloud_email_html_includes_dashboard_when_env_set(monkeypatch):
...
def test_cloud_without_address_falls_back_to_stdout():
    buf = io.StringIO()
    out = D.deliver_briefing(
        _briefing(),
        env=Environment.CLOUD,
    ...
    assert out["channel"] == "stdout"
    # Deployed env carries no subject prefix even when it falls back to stdout.
    assert "[" not in buf.getvalue().splitlines()[0]
