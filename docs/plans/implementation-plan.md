...
- **Deliverable:** `scripts/` or `pipeline/tools/` README for the one-time flow; sample `expert-principles.md` skeleton; contract tests that briefing prompt includes injected guideline text when files exist (mocked S3).
...
- **Prompt:** iterate on `SYSTEM_GUIDELINES` + `build_prompt` constraints; add Phase 6.5 guideline corpus when ready.
...
### Phase 6.7 — Briefing polish pass (post–SES smoke, recurring)

**Purpose:** After real emails land in **staging then prod**, double-check and refine what humans see—separate from shipping new **guideline corpus** (Phase 6.5) or **anomalies** (Phase 8).

- **Email / HTML:** Re-run [briefing-staging-inbox-checklist.md](./briefing-staging-inbox-checklist.md) on major clients (Gmail, Apple Mail, Outlook); fix wrapping, contrast, footer link behavior, and SES **size** limits if templates grow.
- **Prompt engineering:** Tune `SYSTEM_GUIDELINES` and `build_prompt` from misfires; extend [briefing-llm-failure-modes.md](./briefing-llm-failure-modes.md) when you find new patterns; re-evaluate **max_tokens** / model id after Phase 6.5 context grows.
- **Templating:** Optional richer layout (still accessible, still multipart text); wire `BRIEFING_EMAIL_DASHBOARD_URL` via CDK per env when URLs stabilize.
- **Deliverable:** Short dated notes in `docs/plans/` or PR description when you complete a polish cycle—no separate code gate unless regressions appear.
...
- For **every new source**, ship **historical backfill** alongside incremental sync (see **Historical ingestion & backfill** above) so prod/staging are not “empty until the first cron day.”
