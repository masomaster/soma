...
- **Strava (repo slice):** `pipeline/adapters/strava.py` (summary `GET /athlete/activities` → raw JSON per page → `cardio_events` rows), `pipeline/cardio_upsert.py`, `scripts/smoke_strava.py`, Bruno `.bruno/strava/list-athlete-activities.bru`. OAuth token refresh and wiring into the **daily Lambda pipeline** are follow-ups. **Multi-user note:** Strava (and similar) OAuth must eventually land in **per-user** token storage and a **user-facing connect flow** — see Phase 9 **Multi-user rollout — provider connections**.
