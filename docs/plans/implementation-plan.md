...
**When:** After **Phase 7** (production paths + prioritized integrations/backfill) and **Phase 8** (statistical anomalies in the briefing prompt)—so the model already sees **mixed sources** and **anomaly blocks** before you grow prompt context. **Phase 6.6** shipped the first quality bar (sparse recovery, HTML shell, checklist + failure-mode docs). Phase 10 adds **personal + expert narrative context** and then runs **ongoing** SES / prompt / template maturation—not a second “Phase 6” scope.
