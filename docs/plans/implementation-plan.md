...
**Status:** Phases **0–10 repo deliverables complete**. **Single environment** — one `SomaStack` (CloudFormation id `SomaStagingStack`) + one Supabase project; no staging/prod split or Phase 11 cutover. **Dashboard:** **live** at **https://somaapp.streamlit.app** on **[Streamlit Community Cloud](https://streamlit.io/cloud)** (free) — [dashboard-hosting.md](./dashboard-hosting.md). `BRIEFING_EMAIL_DASHBOARD_URL` wired via GitHub variable **`SOMA_DASHBOARD_URL`** (→ CDK `soma:dashboardUrl` on deploy). **Operator next:** apply pending migrations; keep `soma-*` AWS secrets current.
...
**Repo status (2026-07):** Streamlit dashboard with fixture mode, live Supabase Auth, coaching chat, calendar-week tonnage. **Public hosting: live** at **https://somaapp.streamlit.app** on **[Streamlit Community Cloud](https://streamlit.io/cloud)** (free) — see [dashboard-hosting.md](./dashboard-hosting.md). `BRIEFING_EMAIL_DASHBOARD_URL` wired via GitHub variable **`SOMA_DASHBOARD_URL`** (→ `cdk deploy -c soma:dashboardUrl=…`). Operator: apply migrations through `0006`. Multi-user provider OAuth still manual if you ever add users. Next.js PWA deferred.
