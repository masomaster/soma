...
**Repo status (2026-07):** Streamlit dashboard (`dashboard/app.py`) with fixture mode, live Supabase + Auth sign-in, coaching chat + bounded history queries, calendar-week strength tonnage in weekly summary. **Public hosting:** ECS Fargate + ALB (`soma-dashboard`) via CDK — stack output **`DashboardUrl`** (HTTP; custom domain + ACM for HTTPS); briefing email footer wired via **`BRIEFING_EMAIL_DASHBOARD_URL`**. Cloud deploy disables self-service sign-up. Operator: fill **`soma-dashboard`** secret; apply `0005`+. Multi-user provider OAuth still manual (see below). Next.js PWA deferred.
