...
| **Next (dashboard)** | Phase 9: charts for `training_load_*` and optional `effort_*`; wire calendar-week tonnage into dashboard widgets and bounded chat context. **Shipped:** `pipeline/workload_pace.py` — calendar-week ACWR, WoW %, vs 4-wk avg, and green/yellow/red lights for lifting + cardio (dashboard + briefing glance). |
